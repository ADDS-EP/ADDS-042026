...
flowchart TD
    ...
    C -->|Sì| D{"Security Filtering consente Read e Apply?"}
    D -->|No| D1["Correggere gruppo, permessi Read o Apply"]
...
# 23. Caso 1 - Problemi di Security Filtering

## 23.1 Sintomo

La GPO è linkata alla OU corretta, ma l'utente non riceve la configurazione prevista.

Nel caso del LAB07, la GPO interessata è:

```text
GPO_LAB07_GUI_SecurityFiltering_User
```

L'utente che dovrebbe ricevere la GPO è:

```text
LAB\lab07.alfa
```

Il gruppo usato per applicare la GPO è:

```text
GG_LAB07_Restrizioni_Desktop
```

## 23.2 Obiettivo del troubleshooting

In questo caso non si deve verificare solo se l'utente appartiene al gruppo corretto.

Con una GPO filtrata tramite **Security Filtering**, bisogna distinguere due aspetti diversi:

| Aspetto | Domanda da porsi |
|---|---|
| Applicazione della GPO | L'utente o il gruppo hanno il permesso `Apply group policy`? |
| Lettura della GPO | Il client può leggere la GPO durante l'elaborazione? |

Una GPO può non applicarsi anche quando l'utente è membro del gruppo corretto, se la GPO non è leggibile dal client.

## 23.3 Cause principali

Le cause più comuni in questo scenario sono due.

| Causa | Descrizione |
|---|---|
| Utente non incluso nel gruppo corretto | L'utente non è membro del gruppo che ha il permesso `Apply group policy` |
| Permesso di lettura mancante | `Authenticated Users` o `Domain Computers` non hanno almeno il permesso `Read` sulla GPO |

La seconda causa è meno evidente, perché nella scheda **Scope** la GPO può sembrare configurata correttamente.

---

## 23.4 Caso 1A - Utente non incluso nel gruppo usato nel Security Filtering

### Sintomo

La GPO è collegata alla OU corretta, ma l'utente `lab07.alfa` non riceve la configurazione.
...
7. Generare il report:

```cmd
gpresult /h C:\Temp\gpresult_lab07_trouble_security_user.html
start C:\Temp\gpresult_lab07_trouble_security_user.html
```
...
### Verifica rapida del gruppo sul client
...
Risultato atteso quando la configurazione è corretta:

```text
GG_LAB07_Restrizioni_Desktop
```

Se il gruppo non compare, l'utente non ha il gruppo nel token di accesso corrente.

### Correzione

1. Reinserire `lab07.alfa` nel gruppo:

```text
GG_LAB07_Restrizioni_Desktop
```

2. Disconnettere completamente l'utente.
3. Accedere di nuovo con:
...
5. Generare nuovamente il report:

```cmd
gpresult /h C:\Temp\gpresult_lab07_after_group_fix.html
start C:\Temp\gpresult_lab07_after_group_fix.html
```

Verificare che la GPO risulti applicata.

---

## 23.5 Caso 1B - Permesso Read mancante per Authenticated Users o Domain Computers

### Sintomo

L'utente `lab07.alfa` è membro del gruppo corretto, ma la GPO continua a non applicarsi.

Nella scheda **Scope** della GPO è presente:

```text
GG_LAB07_Restrizioni_Desktop
```

Tuttavia il client potrebbe non riuscire a leggere la GPO.

### Perché accade

Nel LAB07 viene rimosso `Authenticated Users` dal **Security Filtering** per fare in modo che la GPO si applichi solo al gruppo:

```text
GG_LAB07_Restrizioni_Desktop
```

Questa scelta è corretta per limitare l'applicazione della GPO.

Il problema nasce se `Authenticated Users` viene rimosso completamente anche dai permessi avanzati della GPO.

Per una GPO utente filtrata, devono essere rispettate entrambe le condizioni:

| Soggetto | Permesso necessario | Scopo |
|---|---|---|
| `GG_LAB07_Restrizioni_Desktop` | `Read` + `Apply group policy` | Consente agli utenti del gruppo di applicare la GPO |
| `Authenticated Users` oppure `Domain Computers` | `Read` | Consente al client di leggere la GPO durante l'elaborazione |

`Authenticated Users` o `Domain Computers` devono avere solo il permesso `Read`.

Non devono avere `Apply group policy`, altrimenti la GPO potrebbe applicarsi a più utenti del previsto.

## 23.6 Verifica dei permessi avanzati della GPO
...
3. Aprire la scheda **Delegation**.
4. Fare clic su **Advanced**.
5. Verificare i permessi.
...
## 23.7 Correzione da GUI

Se `Authenticated Users` non è presente nei permessi avanzati:
...
7. Assegnare solo il permesso:

```text
Read
```

8. Verificare che non sia selezionato:

```text
Apply group policy
```

In alternativa, è possibile assegnare il solo permesso `Read` a:

```text
Domain Computers
```

## 23.8 Verifica dopo la correzione

Su `CLIENT1`, accedere con:
...
```cmd
gpresult /h C:\Temp\gpresult_lab07_after_read_fix.html
start C:\Temp\gpresult_lab07_after_read_fix.html
```

Nel report verificare che tra le GPO applicate compaia:

```text
GPO_LAB07_GUI_SecurityFiltering_User
```

Verificare anche il valore di registro associato alla policy:
...
## 23.9 Interpretazione finale

Se la GPO non si applica, il problema non è sempre l'assenza dell'utente dal gruppo.

Nel Security Filtering bisogna distinguere:
...
deve applicare la GPO.

Il client deve comunque poter leggere la GPO tramite:
...
- Authenticated Users oppure Domain Computers: Read
```
